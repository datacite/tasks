...
Feature: Creating IGSN-DOI from an IGSN Catalogue
        ...
        Scenario: Creating/Editing a IGSN-DOI in any API
            Given the user is authenticated as admin of a IGSN Catalogue
             When the user sends a payload for a DOI metadata
             Then the metadata resourceTypeGeneral default should be set to 'PhysicalObject'
...
Feature: Creating report of IGSN-DOIs
        ...
        Scenario: Obtaining a list of IGSN-DOIs
            Given there are DOI minted by a IGSN Catalogue
             When the user makes a DOI query to our services
              And includes the 'authority' flag set to  'igsn.org'
             Then our query services should return a set of DOI metadata from IGSN-DOIs.
